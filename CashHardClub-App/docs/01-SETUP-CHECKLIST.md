...
| C4 | **OneSignal** | Free plan (up to ~1,000 monthly active users; unlimited mobile push); Growth from $19/month beyond that — check https://onesignal.com/pricing | https://onesignal.com | Push notifications with a dashboard the owners can use themselves | Free plan only has Admin / Team Member roles; the send-but-no-keys role (Editor) needs Professional+ — see doc 04 §7 before inviting Dan and Kalen. Turn on 2FA for every login. |
...
| D7 | Privacy policy page | **Not live yet.** Written (`privacy/index.html` on the website branch) but `https://cashhardclub.com/privacy` returns **404** until that branch is merged and deployed. Hard gate — Apple 5.1.1 and Play both reject a 404 privacy URL. |
| D8 | Support page | **Not live yet.** Written (`support/index.html` on the website branch) but `https://cashhardclub.com/support` returns **404** until merged and deployed. Hard gate — App Store Connect requires a working Support URL (Guideline 1.5). |
| D9 | `events.json` | **Not live yet** — `https://cashhardclub.com/events.json` returns 404 until the website branch is merged, so the Events tab is frozen on the bundled snapshot. The committed file holds one **past** drop, which is a legitimate state to submit with; real upcoming events are optional for submission but are what makes the Upcoming section non-empty (doc 00 explains the fields). |
